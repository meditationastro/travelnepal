...
export const metadata: Metadata = {
  title: 'Learn Nepali Online | Himalaya Retreat Nepal',
  description: 'Learn practical Nepali for travel, trekking, and retreats — phrases, pronunciation, culture notes, and simple daily practice plans.',
};

export default function Page() {
  const pages = getPagesByCategory('nepali');
  const top = pages.slice(0, 12);

  return (
    <main className="mx-auto max-w-6xl px-4 py-12">
      <section className="rounded-3xl border bg-gradient-to-br from-emerald-950 to-stone-900 p-10 text-white shadow-sm">
        <p className="text-xs font-semibold tracking-[0.35em] uppercase text-white/70">Learn Nepali Online</p>
        <h1 className="mt-3 text-4xl font-semibold leading-tight">Learn Nepali Online</h1>
        <p className="mt-4 max-w-2xl text-white/80">Learn practical Nepali for travel, trekking, and retreats — phrases, pronunciation, culture notes, and simple daily practice plans.</p>

        <div className="mt-7 flex flex-wrap gap-3">
          <Link href="/learn/nepali" className="rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-white">Browse all Nepali guides</Link>
      ...
      <section className="mt-12">
        <div className="flex items-end justify-between gap-4">
          ...
          <Link href="/learn/nepali" className="text-sm font-semibold hover:underline">Browse all</Link>
        </div>
      ...
      <section className="mt-12 rounded-3xl border bg-white p-8 shadow-sm">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <Link href="/learn/nepali" className="rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-white">Explore all guides</Link>
