...
export const metadata: Metadata = {
  title: 'ABC Trekking | Himalaya Retreat Nepal',
  description: 'Annapurna Base Camp trekking resources — itinerary, permits, packing, acclimatization, weather, and how to trek ethically with guides and porters.',
};

export default function Page() {
  const pages = getPagesByCategory('treks-abc');
  const top = pages.slice(0, 12);

  return (
    <main className="mx-auto max-w-6xl px-4 py-12">
      <section className="rounded-3xl border bg-gradient-to-br from-sky-950 to-stone-900 p-10 text-white shadow-sm">
        <p className="text-xs font-semibold tracking-[0.35em] uppercase text-white/70">ABC Trekking</p>
        <h1 className="mt-3 text-4xl font-semibold leading-tight">ABC Trekking</h1>
        <p className="mt-4 max-w-2xl text-white/80">Annapurna Base Camp trekking resources — itinerary, permits, packing, acclimatization, weather, and how to trek ethically with guides and porters.</p>

        <div className="mt-7 flex flex-wrap gap-3">
          <Link href="/learn/treks-abc" className="rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-white">Browse ABC trekking guides</Link>
      ...
      <section className="mt-12">
        <div className="flex items-end justify-between gap-4">
          ...
          <Link href="/learn/treks-abc" className="text-sm font-semibold hover:underline">Browse all</Link>
        </div>
      ...
      <section className="mt-12 rounded-3xl border bg-white p-8 shadow-sm">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <Link href="/learn/treks-abc" className="rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-white">Explore all guides</Link>
